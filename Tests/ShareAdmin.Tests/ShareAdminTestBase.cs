...
namespace ShareAdmin.Tests
{
    public abstract class ShareAdminTestBase : AbpIntegratedTestBase<ShareAdminTestModule>
    {
        ...
        protected void UsingDbContext(Action<ShareAdminDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ShareAdminDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ShareAdminDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ShareAdminDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ShareAdminDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ShareAdminDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ShareAdminDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ShareAdminDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ShareAdminDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ShareAdminDbContext, Task<T>> func)
        {
            T result;
